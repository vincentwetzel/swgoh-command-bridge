...
The standalone `ModVisualPreviewWindow` is selected by `--mod-visual-preview` before the normal composition root is created. It is intentionally cache-free so asset/layout verification can run offline.
