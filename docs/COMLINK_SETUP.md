...
- Community recommendation requests identify the client, optionally send the configured Settings contact email as HTTP `From` metadata, use bounded retries and a 2 MiB response-size guard, honor server-provided rate-limit delays, propagate user cancellation, and report privacy-safe endpoint/failure reasons to the optimizer. The optimizer keeps a seven-day freshness window and records the last refresh summary; broader parser hardening and release/legal review remain roadmap work.
