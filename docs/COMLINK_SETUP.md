...
- **Mod visuals are missing:** Run `dotnet run --project src/swgoh-command-bridge.UI/swgoh-command-bridge.UI.csproj -- --mod-visual-preview`. This path does not require Comlink or an account cache. If it reports missing art, verify that the sibling `swgoh-command-bridge-assets/SWGOH_AssetDump` workspace was present when the UI was built.
