...
*There are currently no scheduled long-running in-app agents defined for this project.* Preferred-mod publication is a maintainer-run local command, not an in-app agent or GitHub-hosted worker.
...
Preferred-mod data has a separate best-effort startup refresh. `PreferredModsDatasetService` validates schema and hash before atomically replacing a global aggregate snapshot and preserves the embedded or prior verified dataset when unavailable. The maintainer-run publisher queries local ComLink and publishes only aggregate character/mod distributions, never raw player payloads, ally codes, or credentials.
