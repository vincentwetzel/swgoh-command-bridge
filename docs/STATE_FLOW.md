...
-   **Persistence:** `AppDbContext` stores players, characters, mods, and `swgoh.gg` recommendation JSON in the local SQLite cache. EF Core context, entities, repositories, and the transactional `CacheSchemaMigrator` live under `src/swgoh-command-bridge.Core/Database`; startup repairs missing required tables/columns in older compatible caches, records a supported schema version, and exposes the most recent migration result while the full EF migration history is pending. `AppDataPaths` gives cache, settings, diagnostics, and backup operations one case-stable `SWGOHCommandBridge` application-data directory on every platform. Settings can create and integrity-check restore timestamped SQLite backups, reject unsupported future-schema backups before replacement, and perform confirmation-guarded reset/restore actions that preserve JSON application settings. Versioned secret-safe settings transfer is also available. `SettingsService` stores application configuration as JSON using atomic writes.

-   **Diagnostics:** `DiagnosticsViewModel` reports cache reachability, aggregate record counts, safe configuration metadata, the persisted scraper summary, and bounded in-memory application events. Its export omits full ally codes, account payloads, access keys, and session values.
...
-   **Accounts:** `MainWindowViewModel` lists cached players by ally code, persists the selected cached account as the default, can remove a selected account after confirmation along with its roster/mod rows, reports active character/mod counts on Home, and reloads all feature screens from SQLite without contacting Comlink; an empty active scope shows no account data instead of falling back to every cached account, and syncing remains an explicit user action with visible phase progress.
