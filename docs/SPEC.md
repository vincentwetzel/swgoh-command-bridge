...
-   **Character Catalog:** It ships verified embedded character/ship catalogs, best-effort refreshes them from Comlink, resolves localized names, portrait assets, and alignment, and atomically preserves the previous snapshot when a refresh is rejected. Roster, priority, and optimizer views share portrait rendering with gear/relic frame highlights and initials fallback.
...
-   **Guidance:** It recommends complete mod-set patterns and primary stats by slot, including advice for unmodded slots.
-   **Nuance:** When top-player usage has no clear winner, it shows a preferred option and viable alternatives rather than over-penalizing a close split.
-   **Resilience:** Refresh is silent, bounded, hash-validated, offline-safe, and independent of the user's own Comlink endpoint.
-   **Privacy:** Raw sampled profiles, ally codes, and credentials are not distributed with the app or committed to the repository.
...
-   **Recommendation Engine:**
    ...
    -   **Local Database Caching:** Scraped data is stored in the local SQLite database. To avoid overwhelming `swgoh.gg`, scraping is sequential and incremental, with progress, cancellation, failure reporting, and stale-data checks.
    -   **Preferred GAC Data:** The global preferred dataset supplements locally scraped data with sets and primary distributions derived from high-ranking GAC accounts.
