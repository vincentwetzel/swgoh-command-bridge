...
-   **Full Roster Sync:** Fetch and display a user's complete character and mod inventory for analysis.
...
-   **Recommendation Engine:**
    ...
    -   **Local Database Caching:** The scraped data will be stored in the local SQLite database. To avoid overwhelming `swgoh.gg`, the scraping will be done slowly and incrementally. Once the initial data for a character is fetched, updates will be polled infrequently. Future scope: replace local scraping with a central shared recommendation service.
