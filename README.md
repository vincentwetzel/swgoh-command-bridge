...
- Player sync now maps live `swgoh-comlink` payloads into cached player, character, and mod entities, preserves character priorities, supports cached-account switching, and reports tolerant-parser warnings and phase progress. Scraped `swgoh.gg` recommendations are cached locally with stale-data protection, cancellation, and per-character failure summaries.
