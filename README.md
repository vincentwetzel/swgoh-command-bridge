...
- Mod presentation uses a layered, data-driven visual spec and the shared `swgoh-command-bridge-assets` workspace: inventory rows, selected-mod details, character loadouts, and optimizer assignments render shape, set emblem, pips, 6-dot state, and tier artwork while carrying level metadata in the visual request. The `--mod-visual-preview` launch switch opens an asset/layout verification window without initializing the account cache.
...
To inspect the six shapes, set emblems, tier tinting, pips, and 6-dot variants without connecting to Comlink, run:

```bash
dotnet run --project src/swgoh-command-bridge.UI/swgoh-command-bridge.UI.csproj -- --mod-visual-preview
```

The UI project expects the sibling `swgoh-command-bridge-assets/SWGOH_AssetDump` workspace at its documented relative path. A normal source checkout therefore needs that asset workspace available before building the UI project; Core-only builds do not require it.
...
- [Visual asset and preview notes](docs/FILE_MANIFEST.md#visual-mod-assets)
